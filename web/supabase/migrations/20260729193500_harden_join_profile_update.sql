...
begin
  ...
  if v_entry_no is null then
    raise exception 'You already have the maximum of 3 entries in this pool.';
  end if;
...
revoke all on function public.join_pool_with_invite(
  text,
  uuid,
  text,
  text
...
grant execute on function public.join_pool_with_invite(
  text,
  uuid,
  text,
  text
